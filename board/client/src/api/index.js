...
export const patchBoard = async (id, requestBody) => {
  try {
    ...
    console.error('patchBoard 처리 실패', error)
    throw error
  }
}
...
export const getBoard = async (id) => {
  try {
    ...
    console.error('getBoard 처리 실패', error)
    throw error
  }
}

// 게시글 비활성화 api
export const patchDisableBoardStatus = async (requestBody) => {
  try {
    const response = await api.patch(BOARD_LIST_URL(), requestBody, config)
    return responseHandler(response)
  } catch (error) {
    console.error('patchDisableBoardStatus 처리 실패', error)
    throw error
  }
}
